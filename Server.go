...
func main() {
	...
	go func() {
		log.Printf("Сервер был запущен по адрессу https://localhost:3030/")
		log.Fatal(http.ListenAndServe(":3030", router))
	}()

	<-stopChan
	log.Println("Прекращение работы сервера...")

	saveDataToJSON()

	log.Println("Сервыер был отключен!!!")
}

func saveDataToJSON() {
	file, err := os.Create("db.json")
	if err != nil {
		log.Println("Ошибка открытия или создания db.json:", err)
		return
	}
	defer file.Close()
	...
	if err != nil {
		log.Println("Ошибка декодирования JSON файла:", err)
	}
}
